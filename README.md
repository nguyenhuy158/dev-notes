# dev-notes

Ghi chú dùng chung cho project mới tham khảo, đỡ chọn lại từ đầu.

- [avatar-libs.md](avatar-libs.md) — lib tạo avatar theo tên
- [claudeignore.md](claudeignore.md) — quy định `.claudeignore` cho mọi repo, kèm ví dụ Cloudflare Worker
- [cmdk.md](cmdk.md) — command menu (cmdk)
- [conventions.md](conventions.md) — convention chung
- [date-fns.md](date-fns.md) — xử lý ngày giờ
- [fonts.md](fonts.md) — font đang dùng + đề xuất thay
- [toast-libs.md](toast-libs.md) — lib toast notification
- [zod.md](zod.md) — validate schema
- [zustand.md](zustand.md) — state management
